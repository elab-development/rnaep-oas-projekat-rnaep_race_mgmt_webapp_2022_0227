...
def add_csrf_protection(app: FastAPI, exempt_paths: frozenset[str] = frozenset()):
    ...
    The `csrf_token` cookie is set (non-httpOnly) at login. A cross-site
    attacker page can't read it due to the same-origin policy, so it can't
    reproduce it in the X-CSRF-Token header even though the browser will
    still attach the auth cookie automatically.
    """
    @app.middleware("http")
